...
```
Podemos ver que coinciden bien hasta para valores chicos de n, lo que nos hace dudar la pregunta de que si existe una correccion sencilla, digamos sigue la misma forma, aunque la continuidad de los barplots, los vamos obteniendo en funcion de que n es grande. Cuando n es 10, podemos ver que vale bien las colsa y en el centro parece que faltaria rellenar un poco todavia.
...
potencia <- 1 - (pt(k, df = nu, ncp = centralidad) - pt(-k, df = nu, ncp = centralidad))
cat("Potencia para el test t bilateral para theta = 1: ", potencia)
```
...
```
AGREGAR ESTO EN EL LATEX:
El t test es mas potente que el test de signos, pero no llega a ser mejor que el test de wilcoxon. 
